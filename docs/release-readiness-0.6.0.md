...
This note records the release-readiness decision and publication evidence for
`0.6.0`.

The release commit sets the Maven reactor version to `0.6.0`.
...
policy. The release removes the Modbus experimental label after the Modbus
gates below have merged and been verified on the final release commit.
...
| Function support matrix | PR [#51](https://github.com/Qbsstg/protocol-sdk/pull/51) added the Modbus support matrix; the release commit aligns it with final `0.6.0` behavior. | Complete. |
| API usage guide | PR [#52](https://github.com/Qbsstg/protocol-sdk/pull/52) added TCP, UDP, correlation, exception, and raw fallback usage docs; the release commit updates the dependency version and stability wording. | Complete. |
...
| README stable status | The release commit updates README module status from experimental to stable for the documented Modbus parser scope. | Complete. |
| Release notes | PR [#58](https://github.com/Qbsstg/protocol-sdk/pull/58) added `docs/release-notes-0.6.0.md`; the release commit finalizes the notes. | Complete. |

The final readiness decision was made after all selected Modbus gate PRs merged
into `main`.
...
Run these checks on the final `0.6.0` release commit after all selected gate
PRs have merged:
...
These checks passed on the release branch before tagging:
...
GitHub Actions JDK 8 and JDK 21 verification passed on PR
[#60](https://github.com/Qbsstg/protocol-sdk/pull/60) before merging.

## Publication Results

`0.6.0` was tagged and published on 2026-05-27.

| Item | Result |
| --- | --- |
| Release tag | `v0.6.0` |
| Release commit | `f9070b1e817cfec28c6595527dbcdb58b7bace62` |
| Central deployment | `9f9407e4-8edd-4e7b-aeca-b7d5547d14c9` |
| Central deployment state | `PUBLISHED` |
| External Central availability | `protocol-modbus:0.6.0` and `protocol-sdk:0.6.0` returned `HTTP 200` from Maven Central. |
| External consumer verification | A temporary Maven consumer resolved `protocol-core`, `protocol-iec101`, `protocol-iec103`, `protocol-iec104`, and `protocol-modbus` at `0.6.0` from an isolated local repository and compiled successfully. |

## Current Readiness Decision

`0.6.0` is published and externally resolvable from Maven Central.
