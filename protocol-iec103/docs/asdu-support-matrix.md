...
| 3 | `MEASURANDS_I` | `Iec103MeasuredValue` | Measurands I signed normalized value, quality byte, `FUN`, and `INF` |
| 5 | `IDENTIFICATION` | `Iec103IdentificationValue` | Raw-backed identification payload with stable ASCII projection where applicable |
| 9 | `MEASURANDS_II` | `Iec103MeasuredValue` | Measurands II signed normalized value, quality byte, `FUN`, and `INF` |
...
| --- | --- | --- |
| 4 | `TIME_TAGGED_MEASURANDS_WITH_RELATIVE_TIME` | `FUN`, `INF`, payload bytes, and raw element bytes are preserved; typed measured value plus relative-time metadata is deferred |
...
| Generic data and generic identification | Unknown Type IDs preserve raw bytes for caller-side handling. | Add typed or raw-only constants after representative relay traces define stable public fields. |
| Disturbance directory and disturbance transfer | Unknown Type IDs preserve raw bytes; no disturbance workflow is modeled. | Add raw-only catalog entries first, then typed file-transfer models only after real disturbance records are available. |
...
- Disturbance directory and disturbance-transfer raw-only catalog entries.
- Malformed variable-length identification and generic-data payload cases.
