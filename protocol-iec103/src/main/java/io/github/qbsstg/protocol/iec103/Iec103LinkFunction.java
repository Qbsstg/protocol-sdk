...
public enum Iec103LinkFunction {
    RESET_REMOTE_LINK,
    RESET_USER_PROCESS,
    TEST_LINK_FUNCTION,
    USER_DATA_CONFIRMED,
    ...
    public static Iec103LinkFunction from(boolean primary, int functionCode) {
        if (primary) {
    ...
    private static Iec103LinkFunction primaryFunction(int functionCode) {
    ...
    private static Iec103LinkFunction secondaryFunction(int functionCode) {
        switch (functionCode & 0x0F) {
            case 0:
                return ACK;
            case 1:
